...
const IlmiahCategoryArticles = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await axios.get("/api/articles?category=Ilmiah");
        setArticles(response.data);
      } catch (error) {
    ...
    <div className="text-onSurface">
      <div className="w-full max-w-7xl mx-auto px-4">
        ...
        <div className="flex justify-end mt-4 mr-4">
          <Link to="/artikel/artikel-ilmiah" className="flex items-center">
            <span className="text-md font-medium hover:underline self-start">
              Semua Artikel Ilmiah
            </span>
...
export default IlmiahCategoryArticles;
